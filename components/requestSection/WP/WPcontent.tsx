...
const WPContent: React.FC = () => {
  const { user } = useUser();
  const [requests, setRequests] = useState<RequestWP[]>([]);
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [deleteRequest, setDeleteRequest] = useState<RequestWP | null>(null);
  ...
  const fetchRequests = useCallback(async () => {
    ...
    try {
      const response = await axios.post("/requests/WP/get", {
        userId: user.id,
      });
  ...
  const handleDeleteRequest = (request: RequestWP) => {
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      await axios.delete("/requests/WP/del", {
