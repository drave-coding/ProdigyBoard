...
const DSContent: React.FC = () => {
  const { user } = useUser();
  const [requests, setRequests] = useState<RequestDS[]>([]);
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [deleteRequest, setDeleteRequest] = useState<RequestDS | null>(null);
  ...
  const fetchRequests = useCallback(async () => {
    ...
    try {
      const response = await axios.post("/requests/DS/get", {
        userId: user.id,
      });
  ...
  const handleDeleteRequest = (request: RequestDS) => {
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      await axios.delete("/requests/DS/del", {
